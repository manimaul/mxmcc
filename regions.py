...
class _RegionDatabase:
    ...
    def is_valid_region(self, region):
        return region in self.all_keys
...
_db.add_region(provider_ukho, 'REGION_UK4', 'United Kingdom South West Coast and Ireland East Coast - Irish Sea', map_type_geotiff)


def description_for_region(provider, region):
    region = region.upper()
    return _db.get_description(provider, region)


def map_type_for_region(provider, region):
    region = region.upper()
    return _db.get_map_type(provider, region)


def map_list_for_region(provider, region):
    region = region.upper()
    if _db.provider_has_region(provider, region):
        if provider is privider_noaa:
            reader = NoaaXmlReader(region)
            mps = MapPathSearch(config.noaa_bsb_dir, [map_type_for_region(provider, region)], reader.get_map_files())
            return mps.file_paths
        elif provider is provider_ukho:
            return None  # TODO:
        else:
            mps = MapPathSearch(config.noaa_bsb_dir, [map_type_for_region(provider, region)])
            return mps.file_paths


def lookup_for_provider(provider):
...
if __name__ == '__main__':
    print description_for_region(privider_noaa, 'region_15')
    print map_type_for_region(privider_noaa, 'region_15')
    print directory_for_provider(privider_noaa)
    for map_file in map_list_for_region('noaa', 'region_15'):
        print map_file